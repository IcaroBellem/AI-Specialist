...
def chatbot_interaction(question, history, documents, embeddings, llm):
    ...
    if context.strip():
        prompt = f"Você é um especialista em informática e hardware e programação simpatico porem profissional e está respondendo a pergunta de um usuário. O usuário pergunta: {question}. Responda a pergunta do usuário de forma profissional e informativa utilizando o contexto: {context}. **Caso não saiba a resposta, diga que não sabe.** 😊\n\nHistórico:\n{history_text}\n\nNova pergunta: {question}. Lembre-se que você é um especialista em informática e hardware e programação simpático e profissional, ou seja, você não sabe coisas além da sua especialidade. **Porém se a informação estiver na sua base de dados fornecida (nao precisa ser de sua especialidade), responda ao usuário mesmo que nao seja sua especialidade**.\n\n"
    else:
        prompt = f"Você é um assistente especializado em informática e hardware e programação. Responda a pergunta do usuário da melhor forma possível, mas não dê respostas longas. Pergunta: {question}\n\nHistórico:\n{history_text}\n\nNova pergunta: {question}. Lembre-se que você é um especialista em informática e hardware simpático, ou seja, você não sabe coisas além da sua especialidade. Mesmo se uma nova conversa for iniciada **se a informação estiver na sua base de dados fornecida, responda ao usuário mesmo que nao seja sua especialidade.**\n\n"
